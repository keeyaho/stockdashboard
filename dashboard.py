...
def save_per_history(samsung_f_pe, tsmc_f_pe, forward_ratio, samsung_t_pe, tsmc_t_pe, trailing_ratio):
    ...
    if not all([valid(samsung_f_pe), valid(tsmc_f_pe), valid(samsung_t_pe), valid(tsmc_t_pe)]):
...
# 3. 핵심 수치 모니터링 대시보드 (출력부 추가)
st.subheader("📊 관제 데이터 실시간 현황")

col1, col2 = st.columns(2)
with col1:
    st.metric(label="美 10년물 금리", value=f"{us10:.2f}%" if us10 else "N/A")
    st.metric(label="WTI 원유", value=f"${wti:.2f}" if wti else "N/A")
    st.metric(label="삼성 Forward PER", value=f"{samsung_f_pe:.2f}" if pd.notna(samsung_f_pe) else "N/A")
with col2:
    st.metric(label="美 30년물 금리", value=f"{us30:.2f}%" if us30 else "N/A")
    st.metric(label="구리 선물", value=f"${copper:.2f}" if copper else "N/A")
    st.metric(label="TSM Forward PER", value=f"{tsmc_f_pe:.2f}" if pd.notna(tsmc_f_pe) else "N/A")

st.markdown(f"**💡 PER 상대 비율 (삼성/TSMC):** `{forward_ratio:.4f}`")

if nvda_price:
    st.metric(label="NVDA 종가 (전고점 대비)", value=f"${nvda_price:.2f}", delta=f"{nvda_drawdown:.2f}%")
